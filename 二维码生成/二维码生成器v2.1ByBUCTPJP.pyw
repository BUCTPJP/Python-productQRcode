...
import os
 ...
 #主窗体
window = tk.Tk()
window.title('二维码生成器v2.1 By BUCTPJP')
...
def close():                                                     #解决关闭软件后进程残留问题，关闭进程
    ...
    for pid in pids:
        ...
        if p.name() == '二维码生成器v2.1ByBUCTPJP.exe':
            cmd = 'taskkill /F /IM 二维码生成器v2.1ByBUCTPJP.exe'
            os.system(cmd)
...
tip3 = tk.Label(window,bg='lightcyan',text='请输入二维码想要保存的颜色:',font = ('楷体',12),padx=5,pady=5)
e3 = tk.Entry(window,cursor = 'xterm',relief = SUNKEN,width=30)
tip4 = tk.Text(window,bg = 'aquamarine',font = ('隶书',12),width = 65,height = 4,exportselection = True,relief = RAISED)
tip4.insert(INSERT,'颜色需要输入代码\n可参照如下网址(可以使用CTRL+C复制到浏览器打开)\nhttps://blog.csdn.net/TheLittlePython/article/details/79063779')
tip4.config(state='disabled')
button1 = tk.Button(window,bg='honeydew',text='直接生成',command = produce,relief = RAISED,font=('隶书',14))
button2 = tk.Button(window,bg='honeydew',text='选择logo并生成',command = logo,relief = RAISED,font=('隶书',14))
...
tip4.place(relx=0.35,rely=0.75)
...
showarea.place(relx=0.1,rely=0.75)
